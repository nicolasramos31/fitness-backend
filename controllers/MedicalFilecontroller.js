...
const uploadFile = async (req, res) => {
  try {
    ...
    // Límite 10MB (base64 es ~33% más grande que el original)
    const MAX_BASE64_SIZE = 14 * 1024 * 1024; // ~10MB de archivo real
...
const getAllFiles = async (req, res) => {
  try {
    // Solo coaches pueden ver todos los archivos
...
const getFilesByStudent = async (req, res) => {
  try {
    ...
    const files = await MedicalFile.find({ user: req.params.userId })
      .sort({ createdAt: -1 });
...
const deleteFile = async (req, res) => {
  try {
    ...
    // Solo el dueño del archivo o un coach puede eliminar
